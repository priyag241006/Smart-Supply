  ...
  return (
    <div className="page-animate">
      <div className="page-header">
        ...
        <div className="page-subtitle">Process sales · Stack-based undo · HashMap O(1) lookup</div>
      </div>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <select className="input" value={productId} onChange={e => setProductId(e.target.value)}>
              ...
              {products.map(p => (
                <option key={p.id} value={p.id}>{p.id} · {p.name} (stock: {p.quantity})</option>
              ))}
            </select>
          </div>
          ...
          {selectedProduct && (
            <div style={{ background: 'var(--bg-base)', borderRadius: 8, border: '1px solid var(--border)', padding: '12px 14px', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 16, fontSize: 12, color: 'var(--text-secondary)' }}>
                <span>Stock: <span style={{ color: selectedProduct.quantity <= selectedProduct.threshold ? 'var(--amber)' : 'var(--green)', fontWeight: 500 }}>{selectedProduct.quantity}</span></span>
                <span>Price: <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>₹{selectedProduct.price}</span></span>
                <span>Total: <span style={{ color: 'var(--green)', fontWeight: 500 }}>₹{(selectedProduct.price * qty).toFixed(2)}</span></span>
              </div>
            </div>
          )}
          ...
          {result && (
            <div style={{ marginTop: 14, padding: '10px 14px', borderRadius: 8, background: result.ok ? 'var(--green-bg)' : 'var(--red-bg)', border: `1px solid ${result.ok ? 'rgba(34,211,165,0.2)' : 'rgba(244,63,94,0.2)'}`, display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: result.ok ? 'var(--green)' : 'var(--red)' }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--text-muted)', marginLeft: 'auto' }}>Stack O(1)</span>
          </div>
          {history.length === 0 ? (
              ...
              {history.map((r, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{new Date(r.timestamp).toLocaleTimeString()}</div>
                  </div>
