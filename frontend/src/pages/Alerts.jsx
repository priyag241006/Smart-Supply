    ...
    <div className="page-animate">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div>
          ...
          <div className="page-subtitle">3 alert systems · MinHeap + velocity engine</div>
        </div>
        <button className="btn" onClick={load}><RefreshCw size={13} /> Refresh</button>
      </div>

      <AlertSection title="Fast depletion · Velocity (ArrayList sliding window)" icon={Zap} color="var(--red)"
          ...
          <div key={i} className="alert-item" style={{ borderColor: 'rgba(244,63,94,0.15)', background: a.severity === 'CRITICAL' ? 'var(--red-bg)' : 'transparent' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 14, marginTop: 6, fontSize: 11, color: 'var(--text-muted)' }}>
                ...
                <span>Rate: <strong style={{ color: 'var(--amber)' }}>{a.velocity}/day</strong></span>
                <span>Stockout: <strong style={{ color: 'var(--red)' }}>~{a.daysToStockout}d</strong></span>
              </div>
            </div>
      ...
      <AlertSection title="Low stock · MinHeap (quantity)" icon={TrendingDown} color="var(--amber)"
          ...
          <div key={i} className="alert-item" style={{ borderColor: 'rgba(245,158,11,0.15)' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 14, marginTop: 6, fontSize: 11, color: 'var(--text-muted)' }}>
                ...
                <span>Threshold: <strong style={{ color: 'var(--text-primary)' }}>{a.threshold}</strong></span>
              </div>
            </div>
            <span className="badge" style={{ background: `${sc(a.severity)}18`, color: sc(a.severity) }}>{a.severity}</span>
          </div>
      ...
      <AlertSection title="Expiry alerts · MinHeap (date)" icon={Clock} color="var(--blue)"
        alerts={data?.expiry || []}
        renderAlert={(a, i) => (
          <div key={i} className="alert-item" style={{ borderColor: 'rgba(96,165,250,0.15)' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6 }}>
                Expires: <strong style={{ color: 'var(--text-primary)' }}>{a.expiryDate}</strong>
              </div>
            </div>
